...
###########################################################################
# HOW THE CODE DEDUCT THE LOSS SCORE:
# inp_wav_file -> run
...
class Genetic():
    ...
    def setup_graph(self, input_audio_batch, target_phrase): 
        ...
        with tf.variable_scope('', reuse=tf.AUTO_REUSE):
            ...
            self.a1, self.a2, self.a3 = labels, logits, len_seq
        ...
        func1 = lambda a, b, c, d, e, f: sess.run([ctcloss, target, self.a2, self.a3], 
            feed_dict={inputs: a, len_batch: b, arg2_logits: c, arg1_dense: d, arg2_dense: e, len_seq: f})
        func2 = lambda a, b, c, d, e, f: sess.run([ctcloss, decoded, target, self.a2, self.a3], 
            feed_dict={inputs: a, len_batch: b, arg2_logits: c, arg1_dense: d, arg2_dense: e, len_seq: f})
    ...
    def get_fitness_score(self, input_audio_batch, target_phrase, input_audio, classify=False):
        # print('INPUT AUDIO BATCH:', input_audio_batch)
        # print("TARGET PHRASE:", target_phrase)
        target_enc = np.array([toks.index(x) for x in target_phrase]) 
        if classify:
            ctcloss, decoded, labels, logits, len_seq = self.getctcloss(input_audio_batch, target_enc, decode=True)
            # decoded[0].values = [20, 21, 18, 14,  0, 15,  6,  6, 20, 21, 18, 14,  0, 15,  6,  6, 20,
            #                    21, 18, 14,  0, 15,  6,  6, 20, 21, 18, 14,  0, 15,  6,  6, 20, 21,
            #                    18, 14,  0, 15,  6,  6, 20, 21, 18, 14,  0, 15,  6,  6, 20, 21, 18,
            #                    14,  0, 15,  6,  6, 20, 21, 18, 14,  0, 15,  6,  6, 20, 21, 18, 14,
            #                    0, 15,  6,  6, 20, 21, 18, 14,  0, 15,  6,  6, 20, 21, 18, 14,  0,
            #                    15,  6,  6, 20, 21, 18, 14,  0, 15,  6,  6, 20, 21, 18, 14,  0, 15,
            #                    6,  6, 20, 21, 18, 14,  0, 15,...] 
            # which is the index of the letter 
            all_text = "".join([toks[x] for x in decoded[0].values]) 
            index = len(all_text) // input_audio_batch.shape[0] 
            final_text = all_text[:index]
        else:
            ctcloss, labels, logits, len_seq = self.getctcloss(input_audio_batch, target_enc)
        # print("LABELS", labels)
        print("LOGITS", logits)
...
###########################################################################
# HOW THE CODE DEDUCT THE LOSS SCORE:
# inp_wav_file -> run() -> getfitness_score() -> getctcloss() -> funcs -> setup_graph -> tf.nn.ctc_loss()
# target
# IN SETUP GRAPH
# input (sound files) -> logits ->
# target_phrase -> target
# ctc_loss = tf.nn.ctc_loss(logits, target, ..)
#                             |
#                           deepspeech() -> return what? -> 

essentially deepspeech return logits from the get logits function, in that:
- DeepSpeech.BiRNN is the only deepspeech function that we used
- Logits is a set of probability of each letter at a certain time
Ex:
    {
        a: 0.8,
        b: 0.02,
...
# WHERE DOES DEEPSPEECH HAS A PLAY IN THIS? CAN WE CHANGE IT TO OTHER MODELS? -> YES
# input(196) -> setup_graph -> get_logits (tf_logits.py) -> DeepSpeech (line 106) -> logits(196) -> (basically) predicted text
    
# CAN I USE OTHER MODELS INSTEAD OF DEEPSPEECH? -> Yes by having a meta code and find the text deducted from each model
# WHERE IS TARGET AND PREDICTED BEING COMPARED EXACTLY?
* predicted: best_text (280) -> get_fitness_score() (225) -> str final_text (236) -> decoded (228) -> getctcloss(211) -> input_audio_batch -> pass_in (218) -> funcs[1](pass_in, batch_size, logits_arg2, dense_arg1, dense_arg2, seq_len) (221) -> setup_graph (177) -> inputs(189) -> logits(196) (get_logits(inputs, arg2_logits))                         => ctcloss =  tf.nn.ctc_loss(labels=tf.cast(target, tf.int32), inputs=logits, sequence_length=len_seq)                   
                                                                                                                         target_phrase  -> dense_arg1,2 (215) -> funcs[1](pass_in, batch_size, logits_arg2, dense_arg1, dense_arg2, seq_len) (221) -> setup_graph (177) -> target(197) (target = ctc_label_dense_to_sparse(arg1_dense, arg2_dense, len_batch))=>  

* input filename -> load_wav(input_filename).astype(np.float32) (156) -> self.input_audio (array of frequency. Ex.[0., 0., 0.,...,-328.,-280.,32.]) ->
 self.input_audio_w_bg -> np.expand_dims, np.tile (turn into array of arrays) [[   0.    0.    0. ... -328. -280.   32.]   => GET_FITNESS_SCORE(input_audio_batch) (SEE ABOVE)
                                                                               [   0.    0.    0. ... -328. -280.   32.]
                                                                               [   0.    0.    0. ... -328. -280.   32.]
                                                                               ...
                                                                               [   0.    0.    0. ... -328. -280.   32.]
                                                                               ...
                                                                               [   0.    0.    0. ... -328. -280.   32.]]
TODO:
- Run loop through levienstein dist
= find probability of the target label
